...
/*
#define Scope_Any(func, type)                                      \
    auto UNIQUE_LABEL(CAT(type, lambda))) = [&]() noexcept { func; }; \
    Scope::type<decltype(UNIQUE_LABEL(CAT(type, lambda)))> UNIQUE_LABEL(type)(std::move(UNIQUE_LABEL(CAT(type, lambda)))); \
    (void) 0

#define Scope_Exit(func) Scope_Any(func, Exit)
#define Scope_Success(func) Scope_Any(func, Success)
