...
sendPayload(const std::string & payload)
{
    if (chunkedEncoding) {
        ...
        http->sendHttpChunk(payload, HttpConnectionHandler::NEXT_CONTINUE);
    }
...
{
    if (chunkedEncoding) {
        http->sendHttpChunk("", HttpConnectionHandler::NEXT_CLOSE);
    }
    else if (!keepAlive) {
        http->send("", HttpConnectionHandler::NEXT_CLOSE);
    } else {
        http->send("", HttpConnectionHandler::NEXT_RECYCLE);
    }
