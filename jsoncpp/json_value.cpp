...
#endif
{
    //HACK NaN means Null in this context
...
const Value &
Value::atIndex( UInt index ) const
{
   JSON_ASSERT( type_ == nullValue  ||  type_ == arrayValue );
   if ( type_ == nullValue )
      return null;
...
const Value &
Value::atStr( const char *key ) const
{
   JSON_ASSERT( type_ == nullValue  ||  type_ == objectValue );
   if ( type_ == nullValue )
      return null;
...
const Value &
Value::atStr( const std::string &key ) const
{
   return (*this)[ key.c_str() ];
}
...
const Value &
Value::atStr( const CppTL::ConstString &key ) const
{
   return (*this)[ key.c_str() ];
}
# endif
